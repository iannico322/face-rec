    ...
    return Promise.all(
      Fersons.map(async (label) => {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          let img = await faceapi.fetchImage(URL.createObjectURL(files[i]))
...
useEffect(()=>{
  ...
  return (
    <div className=" min-h-full w-full max-w-[1468px]  flex flex-col justify-center">
        <div className=" border border-border  flex flex-col gap-5 items-center justify-center  py-5 w-full ">
        <form  className=' flex gap-2 flex-col text-accent-foreground px-10 w-full max-w-[500px]' onSubmit={(e:any)=>{
            e.preventDefault()
          ...
          <div>
            <p>Identifier</p>
            <input className=' text-black px-2 py-1  w-full rounded-sm' value={data.id} onChange={(e:any)=>{
              setdata({...data, id: e.target.value })
            }}/>
          </div>
          <div>
            <p>Name</p>
            <input  className=' text-black px-2 py-1  w-full rounded-sm' value={data.name} onChange={(e:any)=>{
              setdata({...data, name: e.target.value })
            }}/>
          </div>
          <div>
            <p>Position</p>
            <input  className=' text-black px-2 py-1 w-full rounded-sm' value={data.position} onChange={(e:any)=>{
              setdata({...data, position: e.target.value })
            }}/>
          </div>

          <div {...getRootProps()} className=" rounded-lg border-2 border-dashed p-4 w-full min-h-[100px] py-3 ">
        
      <input className=' text-accent-foreground'  {...getInputProps()} />

      
      {files.length == 0 ? 
        <p className=' text-accent-foreground'>Drag 'n' drop some Image here, or click to select files</p>
       : ""
        
      }
      <div className=' flex flex-wrap w-full min-h-[200px] gap-2'>


     
      {files.map((file, index) => (
        <div key={index} className=" relative w-[31.88%] h-24 flex  items-center mt-2">
          <img src={URL.createObjectURL(file)} alt={file.name} className=" rounded-md w-full h-24 object-cover" />
          <button
          ...
          <Button variant="outline" className=' text-accent-foreground'>Test</Button>
